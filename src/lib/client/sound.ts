...
class SoundManager {
  ...
  private _voiceCounter = false;
  private _musicEnabled = false;
  private _musicVolume = 0.5;
  private _counter = 0;
  private _lastPoseName: string | null = null;
  ...
  constructor() {
    ...
    } as HowlOptions & { autoUnlock: boolean });
  ...
  get voiceEnabled() { return this._voiceEnabled; }
  set voiceEnabled(val: boolean) { this._voiceEnabled = val; }

  get voiceVolume() { return this._voiceVolume; }
  set voiceVolume(val: number) {
    this._voiceVolume = val;
    this.voice.volume(val);
  }

  get voiceCounter() { return this._voiceCounter; }
  set voiceCounter(val: boolean) { this._voiceCounter = val; }
  ...
  onPose(poseName: string) {
    ...
    if (this._voiceCounter) {
      const countKey = `count${this._counter}` as keyof typeof sprite;
      if (sprite[countKey]) {
  ...
  private queueSprite(spriteName: string) {
    if (this.currentSound && this.voice.playing(this.currentSound as unknown as number)) {
